const $ = new Env('WebP 每日签到'); // 插件名称
$.desc = ['WebP 每日自动签到']; // 插件描述

const token = $.getdata('webp_token'); // 从 Loon 存储中读取 token, key改为 webp_token

!(async () => {
  if (!token) {
    $.msg($.name, '请先设置 Token', '请在 Loon 中配置 webp_token');
    $.done();
    return;
  }
...
async function checkIn() {
  const options = {
    url: 'https://webppt.webp.se/v1/user/check_in', // 请求 URL (已确认)
    headers: {
      'Content-Type': 'application/json',
      'Token': token, // 使用从 Loon 读取的 token
      'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/118.0.0.0 Safari/537.36', // 更常见的版本
      'Referer': 'https://dashboard.webp.se/',
      'Origin': 'https://dashboard.webp.se',
    },
    body: JSON.stringify({}), // 假设请求体为空对象，根据实际情况修改
    // body: "",  // 如果请求体是空字符串，则使用这个
  };
        ...
        if (status === 200) {
          try {
            ...
            if (result && result.success === true) {
              $.msg($.name, '签到成功', result.data);
            } else {
              // 假设返回401表示token过期
              if (status === 401) {
                $.msg($.name, '签到失败', 'Token 已过期，请重新获取');
              }
              else {
                $.msg($.name, '签到失败', `签到失败: ${result.data || body}`);
              }
            }
          } catch (e) {
            $.logErr(e, "json解析错误");
            $.msg($.name, '签到失败', `解析响应出错: ${e}`);
          }
        }
        else{
          $.msg($.name, "签到失败", `状态码: ${status}, 响应: ${body}`);
        }
